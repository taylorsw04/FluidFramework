...
export class RangeMap<K, V> {
	...
	public static union<K, V>(a: RangeMap<K, V>, b: RangeMap<K, V>): RangeMap<K, V> {
		...
		// Use efficient union operation with collision detection
		merged.tree = union<BTree<K, RangeEntry<V>>, K, RangeEntry<V>>(
			a.tree,
			b.tree,
			(_, _valueA, valueB) => {
				// For now, prefer the value from b when there are collisions
				// This maintains the existing behavior where b's values overwrite a's values
				return valueB;
			},
		);
